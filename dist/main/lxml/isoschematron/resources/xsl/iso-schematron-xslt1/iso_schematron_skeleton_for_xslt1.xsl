...
<!-- DPC
  ...
  node()[not(cls::text())] is slower in saxon than *|comment()|processing-instruction()
...
<xsl:template match="*" mode="stylesheetbody">
   ...
   for ZIP archives.
	...
	<xsl:apply-templates select="*[not(cls::iso:ns)] " />
</xsl:template>
...
<xsl:template name="generate-default-rules">
	...
	<!--report the element hierarchy-->
		<axsl:for-each select="ancestor-or-cls::*">
			<axsl:text>/</axsl:text>
			<axsl:value-of select="name(.)"/>
   ...
   <xsl:template match="iso:include">
       ...
       <xsl:choose> 
          ...
          <xsl:when test="string-length( $fragment-id ) &gt; 0">
              ...
              <xsl:if test=" $theFragment_1/cls::iso:schema ">
		   ...
		   <xsl:otherwise>
              ...
              <xsl:if test=" $theFragment_2/cls::iso:schema or $theContainedFragments/cls::iso:schema">
                 <xsl:message>Schema error: Use include to include fragments, not a whole schema</xsl:message>
              </xsl:if>
	...
	or (../iso:phase[@id= $phase]/iso:active[@pattern= current()/@id])">
		...
		<xsl:choose>
		  ...
		  <xsl:when test="$select-contexts='//'">
			...
			<xsl:text>)</xsl:text>
			<xsl:if test="$visit-text='false'">[not(cls::text())]</xsl:if>
		      </xsl:attribute>
