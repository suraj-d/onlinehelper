...
xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:xs="http://www.w3.org/2001/XMLSchema">
        ...
        <xsl:template match="xs:schema">
                ...
                <xsl:call-template name="gatherSchema">
						...
						select="document(/xs:schema/xs:*[cls::xs:include or cls::xs:import or cls::xs:redefine]/@schemaLocation)"/>
                </xsl:call-template>
        </xsl:template>
        <!-- -->
                ...
                <xsl:choose>
                        <xsl:when test="count($schemas) &lt; count($schemas | $includes)">
                                ...
                                <xsl:call-template name="gatherSchema">
                                        ...
                                        <xsl:with-param name="includes" 
										select="document($includes/xs:schema/xs:*[cls::xs:include or cls::xs:import or cls::xs:redefine]/@schemaLocation)"/>
                                </xsl:call-template>
                        </xsl:when>
                        <xsl:otherwise>
